...
get_latest_version() {
    ...
    RELEASE_TAG_TAG=$(curl -s "$RELEASES_URL" | jq -r '.[0].tag_name // ""')

    if [ -z "$RELEASE_TAG_TAG" ]; then
...
RELEASE_CHECK=$(curl -s -o /dev/null -w "%{http_code}" "https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/releases/tags/$RELEASE_TAG")
if [ "$RELEASE_CHECK" != "200" ]; then
    echo "Error: Version $RELEASE_TAG not found."
    exit 1
fi

echo "Installing version: $RELEASE_TAG"

# Fetch and display release details
...
if [ -n "$RELEASE_INFO" ]; then
    ...
    RELEASE_BODY=$(echo "$RELEASE_INFO" | jq -r '.body // ""' | head -5)  # First 5 lines of body
    ...
    if [ -n "$RELEASE_BODY" ] && [ "$RELEASE_BODY" != "null" ]; then
        echo "Release notes (first few lines):"
        echo "$RELEASE_BODY"
    fi
fi
...
RELEASE_URL="https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/releases/tags/$RELEASE_TAG"
echo "Checking available assets for release: $RELEASE_TAG"
...
if [ -z "$ASSETS" ]; then
    ...
    DOWNLOAD_URL="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$RELEASE_TAG/${BINARY_NAME}-${TARGET}.zst"
    ...
    if ! curl -L --output "${BINARY_NAME}.zst" --fail "$DOWNLOAD_URL"; then
        ...
        DOWNLOAD_URL="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$RELEASE_TAG/${BINARY_NAME}-${OS}-${ARCH}.tar.gz"
        echo "Attempting to download from: $DOWNLOAD_URL"

        if ! curl -L --output "${BINARY_NAME}.tar.gz" --fail "$DOWNLOAD_URL"; then
            echo "Failed to download tar.gz format, trying zip..."
            DOWNLOAD_URL="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$RELEASE_TAG/${BINARY_NAME}-${OS}-${ARCH}.zip"
            echo "Attempting to download from: $DOWNLOAD_URL"
        ...
        zstd -d "${BINARY_NAME}.zst" -o "$BINARY_NAME"#!/bin/bash
...
if [ -z "$ASSETS" ]; then
    ...
    if [ -n "$ZST_ASSET" ]; then
        ...
        DOWNLOAD_URL="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$RELEASE_TAG/$ZST_ASSET"
        ...
        DOWNLOAD_URL="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$RELEASE_TAG/$TARGZ_ASSET"
        echo "Downloading from: $DOWNLOAD_URL"
        curl -L --output "$TARGZ_ASSET" --fail "$DOWNLOAD_URL"
        tar -xzf "$TARGZ_ASSET"
    elif [ -n "$ZIP_ASSET" ]; then
        echo "Found zip asset: $ZIP_ASSET"
        DOWNLOAD_URL="https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/$RELEASE_TAG/$ZIP_ASSET"
        echo "Downloading from: $DOWNLOAD_URL"
